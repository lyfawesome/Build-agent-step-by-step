...
最初整理时核对的本地包版本为：`openai 2.53.0`、`langchain 1.3.14`、`langchain-core 1.5.3`、`langgraph 1.2.10`、`pydantic 2.13.4`。仓库当前验证版本以 [`requirements.lock.txt`](../requirements.lock.txt) 为准；框架升级后应重新运行离线测试和模型能力测试。

整理核验（2026-09-07）：本手册、练习册和 demo 的本地链接与 Python 语法检查通过；离线运行 demo 05、06、08、11 成功，08 的示例引用及文件哈希检查通过，仍保持未放行状态。模型脚本需要单独配置 API；语法检查不等于外部模型能力已经验收。
